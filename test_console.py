    ...
    def test_create_fs(self):
        ...
        with self.assertRaises(AttributeError):
            with patch('sys.stdout', new=io.StringIO()) as f:
                HBNBCommand().onecmd("create BaseModel updated_at=2023-10-23T12:00:00.000 "
                                     " created_at=0.0")
        with patch('sys.stdout', new=io.StringIO()) as f:
            HBNBCommand().onecmd('create User email="trurose@gmail.com"'
                                 ' password="trurose"')
        result = f.getvalue().strip()
        self.assertRegex(result, opt)
        email = storage.all()[f'User.{result}'].email
        self.assertEqual(email, "trurose@gmail.com")
        password = storage.all()[f'User.{result}'].password
        self.assertEqual(password, "truerose")
        with patch('sys.stdout', new=io.StringIO()) as f:
        ...
        with patch('sys.stdout', new=io.StringIO()) as f:
            HBNBCommand().onecmd('create City johnny="bravo" number="9"'
                                 ' pi="3.14"')
        result = f.getvalue().strip()
        self.assertRegex(result, opt)
